...
function LoginScreen({ onLogin }) {
  ...
  const handleGoogleSuccess = (credentialResponse) => {
    ...
    // Here we'll do a mock login using a basic payload decode (since we might not have a real client ID setup)
    try {
      const payload = JSON.parse(atob(credentialResponse.credential.split('.')[1]));
      onLogin({ id: `google-${Date.now()}`, name: payload.name || payload.email.split('@')[0], email: payload.email, role: 'student' });
    } catch(e) {
  ...
  return (
    <div className="grid h-full place-items-center bg-soc-bg px-4 text-slate-100 relative">
      <form onSubmit={submit} className="w-full max-w-md rounded-lg border border-soc-border bg-soc-card p-6 shadow-2xl relative z-10">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-xl font-bold text-white">EduSOC</h1>
            <p className="text-sm text-slate-400">{isRegister ? 'Create your account' : 'Institute Member Login'}</p>
          </div>
        </div>
        <div className="space-y-3">
          {showOtp ? (
            <>
              <p className="text-sm text-soc-accent bg-soc-accent/10 p-3 rounded border border-soc-accent/20 mb-4">
                An OTP has been sent to your device via Ntfy. Please enter it below.
              </p>
              <input className="soc-input text-center tracking-[0.5em] font-mono text-lg" type="text" value={otp} onChange={(event) => setOtp(event.target.value)} placeholder="000000" maxLength={6} required />
            </>
          ) : (
            <>
              {isRegister && (
                <>
                  ...
                  <select className="soc-input" value={role} onChange={(e) => setRole(e.target.value)}>
        ...
        <button type="submit" className="soc-btn soc-btn-primary mt-5 w-full justify-center" disabled={loading}>
          ...
          {loading ? 'Processing...' : (showOtp ? 'Verify OTP' : (isRegister ? 'Create Account' : 'Sign In'))}
        </button>
      ...
      {!isRegister && (
        <div className="w-full max-w-md mt-6 rounded-lg border border-soc-border bg-soc-card p-4 shadow-2xl relative z-10">
          <h3 className="text-sm font-semibold text-slate-400 mb-3 text-center">Quick Presentation Logins</h3>
          <div className="flex gap-3">
            <button 
              type="button" 
              onClick={() => { setEmail('analyst@soc.local'); setPassword('admin123'); }}
              className="flex-1 soc-btn soc-btn-ghost justify-center border border-soc-accent/30 text-soc-accent hover:bg-soc-accent/10"
            >
              Sign in as Faculty
            </button>
            <button 
              type="button" 
              onClick={() => { setEmail('student@institute.edu'); setPassword('admin123'); }}
              className="flex-1 soc-btn soc-btn-ghost justify-center border border-soc-border text-slate-300 hover:bg-soc-surface"
            >
              Sign in as Student
            </button>
          </div>
